...
CCA <- function(x, z, typex=c("standard", "ordered"), typez=c("standard","ordered"), penaltyx=NULL, penaltyz=NULL, K=1, niter=15, v=NULL, trace=TRUE, standardize=TRUE, xnames=NULL, znames=NULL, chromx=NULL, chromz=NULL, upos=FALSE, uneg=FALSE, vpos=FALSE, vneg=FALSE, outcome=NULL, y=NULL, cens=NULL, randomised=FALSE){
  ...
  if(!is.null(penaltyx)){
	#print(paste("inside CCA:",penaltyx,penaltyz))
  ...
  out <- CCAAlgorithm(x=x,z=z,v=v,typex=typex,typez=typez,penaltyx=penaltyx,penaltyz=penaltyz,K=K,niter=niter,trace=trace,chromx=chromx,chromz=chromz,upos=upos,uneg=uneg,vpos=vpos,vneg=vneg,randomised=randomised)
  out$outcome <- outcome
  out$call <- call
  out$xnames <- xnames
  out$znames <- znames
...
CCAAlgorithm <- function(x,z,v,typex,typez,penaltyx,penaltyz,K,niter,trace,chromx,chromz,upos,uneg,vpos,vneg,randomised=FALSE){
  if(typez!="ordered"){
  ...
  for(k in 1:K){
    ...
    out <- SparseCCA(xres,zres,v.init[,k],typex,typez,penaltyx, penaltyz,niter,trace, upos, uneg, vpos, vneg,chromx,chromz,randomised)
    coef <- out$d 
    d <- c(d, coef)
...
SparseCCA <- function(x,y,v,typex,typez,penaltyx, penaltyz,niter,trace, upos, uneg, vpos, vneg,chromx,chromz,randomised=FALSE){
  vold <- rnorm(length(v))
  u <- rnorm(ncol(x))
  if(randomised){
	  x0 <- x
	  y0 <- y
	  wx <- 1.0- 0.5*( sample.int(2, size = ncol(x), replace = TRUE)-1 ) 
	  wx <- array( 1,c(nrow(x),1) ) %*% wx
	  wy <- 1.0- 0.5*( sample.int(2, size = ncol(y), replace = TRUE)-1 ) 
	  wy <- array( 1,c(nrow(y),1) ) %*% wy
  }  
  for(i in 1:niter){
...
performOnCluster <- function(count_penalty,i_perm,x,z,sampx,sampz,typex,typez,penaltyxs,penaltyzs,y,outcome,cens,niter,v,trace=FALSE, upos, uneg, vpos, vneg, standardize,chromz,chromx){
	penaltyx <- penaltyxs[count_penalty]
	penaltyz <- penaltyzs[count_penalty]
	i <- i_perm
	#j <- count_penalty
	
	#print( paste("inside pvec:",i,j) )
    for(j in count_penalty){

	 #print( paste("---:",penaltyx,penaltyz) )
     if(trace && .Platform$OS.type!="windows") cat(j,fill=FALSE)
     if(i==1){
     ...
     out <- CCA(x[sampx,],z[sampz,],typex=typex,typez=typez,penaltyx=penaltyxs, penaltyz=penaltyzs,y=y,outcome=outcome,cens=cens,niter=niter,v=v,trace=FALSE, upos=upos, uneg=uneg, vpos=vpos, vneg=vneg, standardize=FALSE,chromz=chromz,chromx=chromx)
     nnonzerous.perms[j,i] <<- sum(out$u!=0)
     nnonzerovs.perms[j,i] <<- sum(out$v!=0)
     if(mean(out$u==0)!=1 && mean(out$v==0)!=1){
       ccperms[j,i] <<- cor(x[sampx,]%*%out$u,z[sampz,]%*%out$v)
...
CCA.permute.bothCluster <- function(x,z,typex,typez,penaltyxs,penaltyzs,niter,v,trace,nperms,standardize,chromx,chromz,upos,uneg,vpos,vneg,outcome,y,cens,cores=2){
	require("parallel")
	require("doParallel")
	...
	lenAloc <- length(penaltyxs)
    ccperms=nnonzerous.perms=nnonzerovs.perms=matrix(NA, lenAloc, nperms)
    ccs=nnonzerous=nnonzerovs=numeric(lenAloc)
    for(i in 1:nperms){
      if(trace && .Platform$OS.type!="windows") cat("\n Permutation ",i," out of ", nperms, " ")
      if(trace && .Platform$OS.type=="windows" && i==1) pb <- winProgressBar(title="Doing Permutations", min=0, max=1, initial=(i/nperms))
      if(trace && .Platform$OS.type=="windows" && i>1) setWinProgressBar(pb, value=(i/nperms)) 
...
#  	vecCluster <- 1:length(penaltyxs)
#  	pvec(vecCluster,performOnCluster,i,x,z,sampx,sampz,typex,typez,penaltyxs,penaltyzs,y,outcome,cens,niter,v, upos, uneg, vpos, vneg, standardize,chromz,chromx, mc.cores=cores)
	
     outObj<-foreach(j=1:lenAloc,.combine=comb,.inorder=TRUE,.multicombine=TRUE)%dopar%{ #length(penaltyxs)
        if(trace && .Platform$OS.type!="windows") cat(j,fill=FALSE)
        if(i==1){
          out <- CCA(x,z,typex=typex,typez=typez,penaltyx=penaltyxs[j], penaltyz=penaltyzs[j],y=y,outcome=outcome,cens=cens,niter=niter,v=v,trace=FALSE, upos=upos, uneg=uneg, vpos=vpos, vneg=vneg, standardize=FALSE,chromz=chromz,chromx=chromx)
          nnonzerous <- sum(out$u!=0)
          nnonzerovs <- sum(out$v!=0)
          if(mean(out$u==0)!=1 && mean(out$v==0)!=1){
            ccs <- cor(x%*%out$u,z%*%out$v)
          } else {
            ccs <- 0
          }
        }
        out <- CCA(x[sampx,],z[sampz,],typex=typex,typez=typez,penaltyx=penaltyxs[j], penaltyz=penaltyzs[j],y=y,outcome=outcome,cens=cens,niter=niter,v=v,trace=FALSE, upos=upos, uneg=uneg, vpos=vpos, vneg=vneg, standardize=FALSE,chromz=chromz,chromx=chromx)
        nnonzerous.perms <- sum(out$u!=0)
        nnonzerovs.perms <- sum(out$v!=0)
        if(mean(out$u==0)!=1 && mean(out$v==0)!=1){
          ccperms <- cor(x[sampx,]%*%out$u,z[sampz,]%*%out$v)
        } else {
          ccperms <- 0
        }
		if(i==1){
			list(nnonzerous=nnonzerous,nnonzerovs=nnonzerovs,nnonzerous.perms=nnonzerous.perms,nnonzerovs.perms=nnonzerovs.perms,ccs=ccs,ccperms=ccperms)
		} else{
			list(nnonzerous.perms=nnonzerous.perms,nnonzerovs.perms=nnonzerovs.perms,ccperms=ccperms)
		}
      } #foreach...
	  ccperms[,i] <- as.numeric(outObj$ccperms)
	  nnonzerous.perms[,i] <- as.numeric(outObj$nnonzerous.perms)
	  nnonzerovs.perms[,i] <- as.numeric(outObj$nnonzerovs.perms)
	  if(i==1){
		  ccs <- as.numeric(outObj$ccs)
		  nnonzerous <- as.numeric(outObj$nnonzerous)
		  nnonzerovs <- as.numeric(outObj$nnonzerovs)
	  }
    } #for(i in 1:nperms)
	#print(ccperms)
    if(trace && .Platform$OS.type=="windows") close(pb)
...
CCA.permuteCluster <- function(x,z,cores=2,typex=c("standard", "ordered"), typez=c("standard","ordered"), penaltyxs=NULL, penaltyzs=NULL, niter=3,v=NULL,trace=TRUE,nperms=25, standardize=TRUE, chromx=NULL, chromz=NULL,upos=FALSE, uneg=FALSE, vpos=FALSE, vneg=FALSE, outcome=NULL, y=NULL, cens=NULL){
  if(ncol(x)<2) stop("Need at least 2 features in data set x.")
  if(ncol(z)<2) stop("Need at least 2 features in data set z.")
  u <- NULL
  typex <- match.arg(typex)
  ...
  lenDim <- 10
  tmpSeq <- seq(.1,.8,len=lenDim)
  if(is.null(penaltyxs) && is.null(penaltyzs) ){
	  penaltyxs <- array(0,c(1,lenDim*lenDim))
	  penaltyzs <- array(0,c(1,lenDim*lenDim))
	  for(i in 1:lenDim){
		  penaltyxs[ ((i-1)*lenDim+1):(i*lenDim)] <- tmpSeq[i]
		  penaltyzs[ ((i-1)*lenDim+1):(i*lenDim)] <- tmpSeq
	  }
	  penaltyxs <- as.numeric(penaltyxs)
	  penaltyzs <- as.numeric(penaltyzs)
  } 
  if(typex=="ordered" && (upos||uneg)) stop("If type=ordered then you cannot require elements of u to be positive or negative!")
  if(typez=="ordered" && (vpos||vneg)) stop("If type=ordered then you cannot require elements of v to be positive or negative!")
  if(length(unique(penaltyxs))==1 && length(unique(penaltyzs))==1){
  ...
  if(length(penaltyxs)==1 && length(penaltyzs)>1) out <- CCA.permute.zonly(x=x,z=z,typex=typex,typez=typez,penaltyx=penaltyxs,penaltyzs=penaltyzs,niter=niter,v=v,trace=trace,nperms=nperms,standardize=standardize,chromx=chromx,chromz=chromz,upos=upos,uneg=uneg,vpos=vpos,vneg=vneg,outcome=outcome,y=y,cens=cens)
  if(length(penaltyxs)>1 && length(penaltyzs)==1) out <- CCA.permute.xonly(x=x,z=z,typex=typex,typez=typez,penaltyxs=penaltyxs,penaltyz=penaltyzs,niter=niter,v=v,trace=trace,nperms=nperms,standardize=standardize,chromx=chromx,chromz=chromz,upos=upos,uneg=uneg,vpos=vpos,vneg=vneg,outcome=outcome,y=y,cens=cens)
  if(length(penaltyzs)>1 && length(penaltyxs)>1) out <- CCA.permute.bothCluster(x=x,z=z,typex=typex,typez=typez,penaltyxs=penaltyxs,penaltyzs=penaltyzs,niter=niter,v=v,trace=trace,nperms=nperms,standardize=standardize,chromx=chromx,chromz=chromz,upos=upos,uneg=uneg,vpos=vpos,vneg=vneg,outcome=outcome,y=y,cens=cens,cores)
  out$call <- call
  out$upos <- upos
  out$uneg <- uneg
  out$vpos <- vpos
